...
public class AvailEvolMonthList {
    ...
    //	private static DecimalFormat df = new DecimalFormat("0.####",new
        ...
        for (AvailEvolMonth month : months) {
            sb.append(
                    month.getDate()
                            ...
                            + "\t"
                            + (month.getZero_five()
                                    + month.getFive_seventyFive()
                                    + month.getSeventyFive_nintyFive())
                            + "\t"
                            + (month.getZero_five()
                                    + month.getFive_seventyFive()
                                    + month.getSeventyFive_nintyFive()
                                    + month.getNintyFive_nintyNine())
                            + "\t"
                            + (month.getZero_five()
                                    + month.getFive_seventyFive()
                                    + month.getSeventyFive_nintyFive()
                                    + month.getNintyFive_nintyNine()
                                    + month.getNintyNine_hundred())
    ...
    public String csvPrintPercent() {
        ...
        for (AvailEvolMonth month : months) {
            sb.append(
                    month.getDate()
                            + "\t"
                            + (new Float(month.getZero_five()) / new Float(month.getNbTotal()))
                                    * 100
                            + "\t"
                            + (new Float(month.getZero_five() + month.getFive_seventyFive())
                                            / new Float(month.getNbTotal()))
                                    * 100
                            + "\t"
                            + (new Float(
                                                    month.getZero_five()
    ...
    private class AvailEvolMonth implements Comparable<AvailEvolMonth> {
        ...
        public AvailEvolMonth(String date) {
            this.date = date;
        }
        ...
        public boolean equals(Object object) {
            if (object != null && object instanceof AvailEvolMonth)
                return ((AvailEvolMonth) object).getDate().equals(this.date);
            return false;
        }
