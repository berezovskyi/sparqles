...
//			
            ...
            log.error("Exception while creating indices for MongoDB collections, {}", ExceptionHandler.logAndtoString(e, true));
        }
    }
    ...
    private boolean insert(String collName, Object e, Schema schema) {
        ...
        try {
            ...
            log.error("INSERT DUPLICATE uri key for {} ({})", e, ExceptionHandler.logAndtoString(ex, true));
            return true;
    ...
    public <V extends SpecificRecordBase> boolean update(V res) {
        ...
        if (res instanceof AvailabilityView)
            return update(COLL_AVAIL_AGG, ((AvailabilityView) res).getEndpoint(), res, res.getSchema(), VIEW_KEY);
        if (res instanceof PerformanceView)
            return update(COLL_PERF_AGG, ((PerformanceView) res).getEndpoint(), res, res.getSchema(), VIEW_KEY);
        if (res instanceof InteroperabilityView)
            return update(COLL_FEAT_AGG, ((InteroperabilityView) res).getEndpoint(), res, res.getSchema(), VIEW_KEY);
        if (res instanceof DiscoverabilityView)
            return update(COLL_DISC_AGG, ((DiscoverabilityView) res).getEndpoint(), res, res.getSchema(), VIEW_KEY);
        
        if (res instanceof Endpoint)
            return update(COLL_ENDS, ((Endpoint) res), res, res.getSchema(), EP_KEY);
        if (res instanceof EPView)
            return update(COLL_EP_VIEW, ((EPView) res).getEndpoint(), res, res.getSchema(), VIEW_KEY);
    ...
    private boolean update(String collName, Endpoint ep, Object e, Schema schema, String key) {
        ...
        try {
            ...
            log.error("INSERT DUPLICATE uri key for {} ({})", ep.getUri(), ExceptionHandler.logAndtoString(ex, true));
            return true;
        } catch (MongoException ex) {
    ...
    public boolean remove(Endpoint ep, Class cls) {
        ...
        try {
            ...
            log.error("REMOVE DUPLICATE uri key for {} ({})", ep.getUri(), ExceptionHandler.logAndtoString(ex, true));
            return true;
        } catch (MongoException ex) {
            log.error("REMOVE MongoDB Exception: {}: {}", ep.getUri(), ExceptionHandler.logAndtoString(ex, true));
        } catch (Exception ex) {
            log.error("REMOVE Exception: {} {}", ep.getUri(), ExceptionHandler.logAndtoString(ex, true));
        }
        return false;
    }
