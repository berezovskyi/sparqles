...
public class MongoDBManager {
  ...
  public void initAggregateCollections() {
    String[] cols = {
      COLL_AVAIL_AGG, COLL_PERF_AGG, COLL_DISC_AGG, COLL_CALC_AGG, COLL_FEAT_AGG, COLL_FEAT_AGG
    };
    for (String col : cols) {
      DBCollection c = db.getCollection(col);
      c.drop();
  ...
  private boolean insert(String collName, Object e, Schema schema) {
    ...
    try {
      ...
      c.insert(dbObject, WriteConcern.ACKNOWLEDGED);
  ...
  public <V extends SpecificRecordBase> boolean update(V res) {
    ...
    if (res instanceof AvailabilityView)
      return update(
          COLL_AVAIL_AGG, ((AvailabilityView) res).getEndpoint(), res, res.getSchema(), VIEW_KEY);
    if (res instanceof PerformanceView)
      return update(
          COLL_PERF_AGG, ((PerformanceView) res).getEndpoint(), res, res.getSchema(), VIEW_KEY);
    ...
    if (res instanceof DiscoverabilityView)
      return update(
          COLL_DISC_AGG, ((DiscoverabilityView) res).getEndpoint(), res, res.getSchema(), VIEW_KEY);
    if (res instanceof CalculationView)
      return update(
          COLL_CALC_AGG, ((CalculationView) res).getEndpoint(), res, res.getSchema(), VIEW_KEY);
  ...
  public <T> Iterator<T> getResultsIterator(Endpoint ep, Class<T> cls, Schema schema) {
    ...
    if (v != null && v[0] != null && v[1] != null) return scanIterator(ep, v[0], cls, schema, v[1]);
    else {
      log.warn("Collection for {} unknown", cls);
    }
    ...
    try {
      ...
      while (curs.hasNext()) {
        ...
        try {
          ...
          log.error(
              "GET RESULT Exception: {} {}", ep.getUri(), ExceptionHandler.logAndtoString(e, true));
        }
      }
    } finally {
  ...
  private DBObject getObject(Object o, Schema s) {
    ...
    try {
      ...
      log.error(
          "GET OBJECT Exception: {} {}", o.getClass(), ExceptionHandler.logAndtoString(e1, true));
    }
    return null;
  }
  ...
  public boolean remove(Endpoint ep, Class cls) {
    ...
    try {
      ...
      c.remove(q, WriteConcern.ACKNOWLEDGED);
  ...
  public long getFirstAvailabitlityTime() {
    ...
    try {
      ...
      result =
          Long.parseLong(((DBObject) cursor.next().get("endpointResult")).get("start").toString());
    } finally {
      cursor.close();
    }
    return result;
