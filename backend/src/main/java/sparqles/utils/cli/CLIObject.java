...
public abstract class CLIObject {
    ...
    /**
     * add all Option(Groups) to this object Note: The help flag is set automatically ("?")
     *
     * @param opts
     */
    protected abstract void addOptions(Options opts);
    ...
    public CommandLine verifyArgs(String[] args) {
        ...
        if (cmd != null
                && (cmd.hasOption(ARGUMENTS.PARAM_HELP)
                        || cmd.hasOption(ARGUMENTS.PARAM_HELP1[ARGUMENTS.SHORT_ARG])
                        || cmd.hasOption(ARGUMENTS.PARAM_HELP1[ARGUMENTS.LONG_ARG]))) {
