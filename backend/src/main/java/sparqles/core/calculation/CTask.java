...
public class CTask extends EndpointTask<CResult> {
    ...
    public CResult process(EndpointResult epr) {
        ...
        if (ping) {
            ...
            try {
                ...
                if (triples != -1 && distinctSubjects != -1)
                    relationshipSpecialty =
                            calculateRelationshipSpecialty(_epURI, triples, distinctSubjects);
            } catch (Exception e) {
                log.warn("[Error details: {}]", e.toString());
            }
            ...
            Resource voidDatasetDescription =
                    modelVoID.createResource(voidNS + "DatasetDescription");
            ...
            for (int i = 0; i < exampleResourceList.size(); i++)
                endpointEntityVoiD.addProperty(
                        voidexampleResource,
                        modelVoID.createResource(exampleResourceList.get(i).toString()));
    ...
    public java.util.List<java.lang.CharSequence> executeQuery(
            String endpointURL, String queryText) {
    ...
    public double calculateCoherence(String endpointUrl) {
        ...
        for (String type : types) {
            ...
            double weightedCoverage =
                    (typePredicates.size() + typeInstancesSize) / weightedDenomSum;
    ...
    public static long getTypeInstancesSize(String type, String endpoint) {
        ...
        try {
            ...
            try (QueryExecution qExec = QueryManager.getExecution(endpoint, queryString)) {
                ...
                while (res.hasNext()) {
                    typeInstancesSize =
                            Long.parseLong(res.next().get("cnt").asLiteral().getString());
                }
            }
        } catch (Exception e) {
            log.warn("[Error executing SPARQL query for {}]", endpoint);
