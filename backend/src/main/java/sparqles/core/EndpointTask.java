...
public abstract class EndpointTask<V extends SpecificRecordBase> implements Task<V> {
    ...
    public V call() throws Exception {
        ...
        try {
            ...
            if (_dbm != null) i_succ = _dbm.insert(v);

            // write to file
            if (_fm != null) f_succ = _fm.writeResult(v);

            // analyse the results
            if (_analytics != null) a_succ = _analytics.analyse(v);

            log.info(
                    "EXECUTED {} in {} ms (idx:{}, disk:{}, analysed:{})",
                    _id,
                    end - start,
                    i_succ,
                    f_succ,
    ...
    public abstract V process(EndpointResult epr);
